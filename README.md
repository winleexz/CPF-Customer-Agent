...
o	Get Customer Profile Tool: The agent calls the Cypher template and fetches Customer X’s age property 
o	Reasoning & Output: The agent combines these signals. Instead of a generic eligibility criterion, the agent generates a definitive, profile verified "Yes/No" eligibility draft for the executive
...
•	Scenario B: Dynamic Calculations
o	Query: Customer X asks: 'How much do I have to top up to hit FRS in 2027?'
o	Similarity Search Tool: The agent embeds the question text and searches the vector index. It returns the node detailing the 2027 FRS requirements
o	Get Customer Profile Tool: The agent calls the Cypher template for Customer ID X and pulls the customer's accountBalance
